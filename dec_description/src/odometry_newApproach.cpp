...
void odomcallback(const dec_description::odom_data::ConstPtr& odom_vl)
{
 ...
 deltaLeft =  odom_vl->vx - _PreviousLeftEncoderCounts;
 deltaRight = odom_vl->vy - _PreviousRightEncoderCounts;
 ...
 if ( th <= -PI)
 th += TwoPI;
 _PreviousLeftEncoderCounts = odom_vl->vx;
 _PreviousRightEncoderCounts = odom_vl->vy;
...
int main(int argc, char **argv)
{
 ...
 ros::NodeHandle n;
    ros::Subscriber sub = n.subscribe("/vl", 100, odomcallback);
